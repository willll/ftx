...
#define CHECK_FL_INIT()     { if (fat_get_global()->filelib_init==0) fl_init(); }
...
{
    FL_GLOBAL* fg = fat_get_global();

    // Allocate free file
    struct fat_node *node = fat_list_pop_head(&fg->free_file_list);

    // Add to open list
    if (node)
        fat_list_insert_last(&fg->open_file_list, node);
...
static int _check_file_open(FL_FILE* file)
{
    ...
    // Compare open files
    fat_list_for_each(&fg->open_file_list, node)
    {
...
static void _free_file(FL_FILE* file)
{
    ...
    fat_list_remove(&fg->open_file_list, &file->list_node);

    // Add to free list
    fat_list_insert_last(&fg->free_file_list, &file->list_node);
}
...
static int _open_directory(char *path, uint32 *pathCluster)
{
    ...
    startcluster = fatfs_get_root_cluster(&fg->fs);
    ...
    for (sublevel=0;sublevel<(levels+1);sublevel++)
    {
        ...
        if (fatfs_get_file_entry(&fg->fs, startcluster, currentfolder,&sfEntry))
        {
...
static int _create_directory(char *path)
{
    ...
    if (file->path[0] == 0)
        file->parentcluster = fatfs_get_root_cluster(&fg->fs);
    else
    {
    ...
    if (fatfs_get_file_entry(&fg->fs, file->parentcluster, file->filename,&sfEntry) == 1)
    {
        _free_file(file);
        return 0;
    }
    ...
    if (!fatfs_allocate_free_space(&fg->fs, 1, &file->startcluster, 1))
    {
        _free_file(file);
        return 0;
    }
    ...
    for (i=0;i<fg->fs.sectors_per_cluster;i++)
    {
        if (!fatfs_write_sector(&fg->fs, file->startcluster, i, file->file_data_sector))
        {
            _free_file(file);
            return 0;
        }
    ...
    do
    {
        ...
        if (fatfs_sfn_exists(&fg->fs, file->parentcluster, (char*)file->shortfilename) == 0)
            break;
    ...
    if (tailNum == 9999)
    {
        ...
        fatfs_free_cluster_chain(&fg->fs, file->startcluster);
    ...
    if (fatfs_sfn_exists(&fg->fs, file->parentcluster, (char*)file->shortfilename))
    {
        // Delete allocated space
        fatfs_free_cluster_chain(&fg->fs, file->startcluster);
    ...
    // Add file to disk
    if (!fatfs_add_file_entry(&fg->fs, file->parentcluster, (char*)file->filename, (char*)file->shortfilename, file->startcluster, 0, 1))
    {
        // Delete allocated space
        fatfs_free_cluster_chain(&fg->fs, file->startcluster);
...
static FL_FILE* _open_file(const char *path)
{
    ...
    if (fatfs_get_file_entry(&fg->fs, file->parentcluster, file->filename,&sfEntry))
...
static FL_FILE* _create_file(const char *filename)
{
    ...
    // Add file to disk
    if (!fatfs_add_file_entry(&fg->fs, file->parentcluster, (char*)file->filename, (char*)file->shortfilename, file->startcluster, 0, 0))
    {
        // Delete allocated space
        fatfs_free_cluster_chain(&fg->fs, file->startcluster);
...
static uint32 _read_sectors(FL_FILE* file, uint32 offset, uint8 *buffer, uint32 count)
{
    ...
    ClusterIdx = offset / fg->fs.sectors_per_cluster;
    Sector = offset - (ClusterIdx * fg->fs.sectors_per_cluster);

    // Limit number of sectors read to the number remaining in this cluster
    if ((Sector + count) > fg->fs.sectors_per_cluster)
        count = fg->fs.sectors_per_cluster - Sector;
    ...
    else
    {
        ...
        for ( ;i<ClusterIdx; i++)
        {
            ...
            if (!fatfs_cache_get_next_cluster(&fg->fs, file, i, &nextCluster))
            {
                // Scan file linked list to find next entry
                nextCluster = fatfs_find_next_cluster(&fg->fs, Cluster);

                // Push entry into cache
                fatfs_cache_set_next_cluster(&fg->fs, file, i, nextCluster);
            }
    ...
    lba = fatfs_lba_of_cluster(&fg->fs, Cluster) + Sector;

    // Read sector of file
    if (fatfs_sector_read(&fg->fs, lba, buffer, count))
        return count;
    else
        return 0;
}
...
void fl_init(void)
{
    ...
    fat_list_init(&fg->free_file_list);
    fat_list_init(&fg->open_file_list);

    // Add all file objects to free list
    for (i=0;i<FATFS_MAX_OPEN_FILES;i++)
        fat_list_insert_last(&fg->free_file_list, &(fg->files)[i].list_node);
...
int fl_attach_media(fn_diskio_read rd, fn_diskio_write wr)
{
    ...
    fg->fs.disk_io.read_media = rd;
    fg->fs.disk_io.write_media = wr;

    // Initialise FAT parameters
    if ((res = fatfs_init(&fg->fs)) != FAT_INIT_OK)
    {
        FAT_PRINTF(("FAT_FS: Error could not load FAT details (%d)!", res));
        return res;
    }
...
static uint32 _write_sectors(FL_FILE* file, uint32 offset, uint8 *buf, uint32 count)
{
    ...
    ClusterIdx = offset / fg->fs.sectors_per_cluster;
    SectorNumber = offset - (ClusterIdx * fg->fs.sectors_per_cluster);

    // Limit number of sectors written to the number remaining in this cluster
    if ((SectorNumber + count) > fg->fs.sectors_per_cluster)
        count = fg->fs.sectors_per_cluster - SectorNumber;
    ...
    else
    {
        ...
        if (Cluster == FAT32_LAST_CLUSTER)
        {
            ...
            if (!fatfs_add_free_space(&fg->fs, &LastCluster,  (TotalWriteCount + fg->fs.sectors_per_cluster -1) / fg->fs.sectors_per_cluster))
                return 0;
    ...
    lba = fatfs_lba_of_cluster(&fg->fs, Cluster) + SectorNumber;

    if (fatfs_sector_write(&fg->fs, lba, buf, count))
        return count;
    else
        return 0;
}
...
void fl_fclose(void *f)
{
    ...
    if (file)
    {
        ...
        if (file->filelength_changed)
        {
            ...
            fatfs_update_file_length(&fg->fs, file->parentcluster, (char*)file->shortfilename, file->filelength);
#endif
            file->filelength_changed = 0;
        }
...
int fl_remove( const char * filename )
{
    ...
    {
        // Delete allocated space
        if (fatfs_free_cluster_chain(&fg->fs, file->startcluster))
        {
            // Remove directory entries
            if (fatfs_mark_file_deleted(&fg->fs, file->parentcluster, (char*)file->shortfilename))
            {
...
FL_DIR* fl_opendir(const char* path, FL_DIR *dir)
{
    ...
    if (levels == 0)
        cluster = fatfs_get_root_cluster(&fg->fs);
    // Find parent directory start cluster
    else
        res = _open_directory((char*)path, &cluster);

    if (res)
        fatfs_list_directory_start(&fg->fs, dir, cluster);
...
int fl_readdir(FL_DIR *dirls, fl_dirent *entry)
{
    ...
    res = fatfs_list_directory_next(&fg->fs, dirls, entry);
...
int fl_format(uint32 volume_sectors, const char *name)
{
    ...
    return fatfs_format(&fg->fs, volume_sectors, name);
}
