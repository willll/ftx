...
int cdfs_open(char *path, file_struct *file)
{
    char *p;
    char dir_name[32];
    dir_rec_struct dir_rec;
    ...
    /* Clear informations that may remain from a
     * previous CD-ROM file operation.
     * Example : open existing file, then open missing file.
     * (I know that return codes are here for that.)
     */
    memset((void*)file, 0, sizeof(file_struct));
    ...
    while(!done)
    {
        ...
        for (;;)
        {
            ...
            /* Remove trailing ";" character, if exist. */
            for(i=0; i<32; i++)
            {
                if(dir_rec.name[i] == ';')
                {
            ...
            if (strcasecmp(dir_rec.name, dir_name) == 0 ||
                strncasecmp(dir_rec.name, dir_name, strlen(dir_name)) == 0)
            {
                lba = dir_rec.lba;
...
int cdfs_filelist(scdf_file_entry_t* list_ptr, unsigned long list_count, unsigned long list_offset, char* path, unsigned long* file_count_ptr)
{
    char *p;
    char dir_name[32];
    dir_rec_struct dir_rec;
            ...
            if(done)
            {
                ...
                else
                {
                    ...
                    && (file_index < (list_offset+list_count)))
                    {
                        ...
                        if(dir_rec.flags & 0x02)
                        {
                            list_ptr[list_index].flags |= FILEENTRY_FLAG_FOLDER;
                        }
                        strncpy(list_ptr[list_index].filename, dir_rec.name, SC_FILENAME_MAXLEN);
